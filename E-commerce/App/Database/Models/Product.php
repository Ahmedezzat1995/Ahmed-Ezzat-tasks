...
//read new products 
public function read_new_products()
{
    $quary="SELECT * from products where status= 1 order by `created_at`DESC LIMIT 4" ;
    return  $this->conn->query($quary) ;
}

//read most orders
public function read_most_order()
{
    $quary="SELECT * from most_order  LIMIT 4" ;
    return  $this->conn->query($quary) ;
}
